...
static void CalculateMeshNormals(mesh3ds_t* mesh)
{
   ...
   if (mesh->smooth == NULL)
   {
	   mesh->norms = (float(*)[3])malloc(sizeof(float) * 3 * mesh->vertCount);
	   if (mesh->norms == NULL)
		{
		   return;
		}
	   
	   //reserve memory. Every vertex gets a normal.
	   memset(mesh->norms, 0, sizeof(float) * 3 * mesh->vertCount);
	   ...
	   for (i = 0; i < mesh->triCount; i++)
	   {
			...
			VEC_Cross(v1, v2, normal);
			VEC_Normalizef( normal );
			v0normal = mesh->norms[mesh->tris[i][0]];
			v1normal = mesh->norms[mesh->tris[i][1]];
			v2normal = mesh->norms[mesh->tris[i][2]];
			VEC_AddTo( v0normal, normal );
			VEC_AddTo( v1normal, normal );
			VEC_AddTo( v2normal, normal );			
	   }
	   for (i = 0; i < mesh->vertCount; i++)
	   {
